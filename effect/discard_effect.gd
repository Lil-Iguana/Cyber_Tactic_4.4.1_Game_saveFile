...
func execute(targets: Array[Node]) -> void:
	...
	if not Events.card_chosen.is_connected(player_handler.discard_card_with_signal):
		Events.card_chosen.connect(player_handler.discard_card_with_signal, CONNECT_ONE_SHOT)
		
	Events.hand_choice_requested.emit("Discard a card")
