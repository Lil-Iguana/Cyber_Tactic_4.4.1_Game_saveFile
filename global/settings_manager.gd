...
func set_fullscreen(value: bool):
	...
	if value:
		...
		if RESOLUTIONS.has(config["resolution"]):
			DisplayServer.window_set_size(RESOLUTIONS[config["resolution"]])
			center_window()
...
func center_window():
	...
	var centered_position = Vector2i((screen_size - window_size) / 2.0)
	DisplayServer.window_set_position(centered_position)

func is_resolution_changeable() -> bool:
	# Resolution can only be changed when not in fullscreen
	return not config["fullscreen"]
